...
const findSelectedGiftPackageInStore = () => {
  if (cart.value.gift.title) {
    return gift.find((item) => item.title === cart.value.gift.title);
  }
  return gift[0];
};
...
const selectedGiftPackage = ref(findSelectedGiftPackageInStore());
...
watch(
  () => selectedGiftPackage.value,
  (newValue) => {
    cart.value.gift = { ...newValue! };
  },
  { immediate: true }
);
</script>
